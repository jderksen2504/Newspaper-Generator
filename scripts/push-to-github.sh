...
# ---------------------------------------------------------------------------
if [[ ! -d "desktop/src/fonts" ]] || [[ -z "$(ls -A desktop/src/fonts 2>/dev/null)" ]]; then
  echo ""
  echo "=== Lade Web-Fonts herunter (lokal, nicht im Repo) ==="
  if command -v node &>/dev/null; then
    node desktop/scripts/download-fonts.js || echo "WARNUNG: Font-Download fehlgeschlagen"
  else
...
- Web-App (Next.js 16 + TypeScript + Puppeteer) im Root
- Desktop-App (Tauri 2.0 + Rust + Playwright Sidecar) in desktop/

Features:
- Live-Vorschau mit Fraktur/Blackletter/Serif/Italic-Schriften
- Pro-Zone-Schriftgrößen + globaler Zoom
